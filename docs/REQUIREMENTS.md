...
REQ-070 | FALSE | Production input requirements must round up fractional input quantities to the nearest whole unit based on total production | Prevents underconsumption of discrete inputs and ensures correct economic modeling of BOM usage | Domain Rule | Production | RequiredInputUnits = CEILING(InputQty × OutputUnits)
...
| REQ-081 | TRUE | System must compute production at full capacity without internal bottleneck constraints | Ensures deterministic production prior to imbalance resolution | Functional | Production | Bottleneck handling occurs via clearing layer |
| REQ-082 | TRUE | Clearing allocations plus clearing remainder must equal absolute net imbalance per product and quality level | Ensures complete and consistent imbalance resolution | Invariant | Clearing | enforced in stage_clearing_allocation |
| REQ-084 | TRUE | System must compute production at full capacity independent of input availability, with all upstream shortages resolved through clearing channels | Bottleneck-constrained production introduces implicit routing, prioritization, and non-deterministic behavior | Domain Rule | Production | Production is modeled as continuous and independent of integer run constraints. Input availability does not constrain production |
| REQ-085 | TRUE | System must expand BOM consumption recursively across all dependency levels until only terminal products remain | Single-level consumption understates upstream resource requirements | Functional | Production | Enforced via recursive BOM expansion in product_bom_consumption |
| REQ-086 | TRUE | Product BOM graph must be acyclic; any cycle must result in validation failure | Cycles create infinite recursion and undefined consumption | Validation | Production | Enforced in validator before pipeline execution |
| REQ-087 | TRUE | Clearing must fully resolve all imbalance; remaining_units_per_hour must be zero for all products | Partial clearing creates undefined system state and violates deterministic resolution | Invariant | Clearing | Enforced in stage_clearing_allocation |
| REQ-088 | TRUE | Output tables may be empty only when upstream inputs produce no rows; tables must not be empty when upstream data implies required results | Distinguishes valid no-op runs from incomplete execution | Validation | Engine | Applies across all output tables |
| REQ-089 | TRUE | Retail channel allocation must only occur when explicitly defined in clearing_plan; absence of retail channel implies zero retail allocation | Prevents implicit retail behavior and preserves explicit configuration | Domain Rule | Clearing | Retail capacity evaluated only when retail channel present |
| REQ-090 | TRUE | Missing logically required data must result in execution failure even if schema validation passes | Prevents silent incorrect outputs due to incomplete data relationships | Validation | Engine | Applies to BOM presence, clearing plan completeness, and required linkage |
| REQ-091 | TRUE | System must perform complete structural validation and typing of all input and reference tables prior to pipeline execution, guaranteeing structurally valid data for all downstream stages | Without a centralized validation boundary, stages must defensively validate inputs, leading to duplication and inconsistency | Validation | Engine | Stage logic may assume schema correctness, typing, and structural completeness; invariants enforced separately |
| REQ-092 | TRUE | Each pipeline stage must enforce its own behavioral invariants and guarantee correctness of its output before passing data downstream | Ensures correctness is maintained throughout the pipeline without re-validating structure | Invariant | Engine | Invariants include conservation rules, allocation correctness, and domain-specific constraints |
REQ-093 | TRUE | System must compute production and BOM consumption as continuous rates without requiring integer input rounding | Supports game-realistic planning where fractional consumption represents average sustained usage over time | Domain Rule | Production | InputQty × OutputUnits may be fractional; integer execution handled externally
| REQ-094 | TRUE | All table structures must be defined in external contract files and must not be hardcoded in the engine | Hardcoded table definitions prevent extensibility and require code changes for new tables | Constraint | Engine | Contracts are the single source of truth for table structure |
| REQ-095 | TRUE | Required and non-empty table semantics must be defined in contract metadata rather than engine logic | Hardcoded validation rules create drift between schema and runtime behavior | Validation | Engine | Replaces hardcoded checks in run.py |
| REQ-096 | TRUE | Contract files must fully define table shape including fields, types, constraints, and keys | Partial schemas require defensive logic and create ambiguity | Constraint | Contracts | Validator assumes complete schema |
| REQ-097 | TRUE | Engine must dynamically discover contract tables from contract files instead of fixed filenames | Static schema loading prevents extensibility | Functional | Engine | Enables “add YAML without code changes” |
| REQ-098 | TRUE | All UI lookup mappings must be defined in contract files and not duplicated in code | Hardcoded lookups create inconsistency and maintenance risk | Constraint | Tooling | Eliminates LOOKUPS and LOOKUP_TO_RANGE constants |
| REQ-099 | TRUE | All contract files must explicitly declare their contract type | Implicit interpretation leads to parsing ambiguity | Constraint | Contracts | Uses `kind: table`, `kind: lookup_mapping` |
